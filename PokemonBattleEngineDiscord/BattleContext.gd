...
static var _activeBattlesLockObj : object = new();
static var List<BattleContext> _activeBattles = new();
static var Dictionary<SocketUser, BattleContext> _activeBattlers = new(DiscordComparers.UserComparer);
static var Dictionary<ITextChannel, BattleContext> _activeChannels = new(DiscordComparers.ChannelComparer);
static var Dictionary<IGuild, List<BattleContext>> _activeGuilds = new(DiscordComparers.GuildComparer);
static var ulong _battleCounter = 1;
