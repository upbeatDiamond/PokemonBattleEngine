...
#
class PBEBattle: ## Merge Battle into this.
	...
	static func AreActionsValid(trainer:PBETrainer, actions: Array[PBETurnAction], invalidReason:String) -> bool:
		if (trainer.Battle._battleState != PBEBattleState.WaitingForActions):
			push_error("ERR UNSPECIFIED") #throw InvalidOperationException($"{nameof(BattleState)} must be {PBEBattleState.WaitingForActions} to validate actions.");
			...
			if (action.Decision == PBETurnDecision.Fight && pkmn.GetMoveTargets(action.FightMove) == PBEMoveTarget.RandomFoeSurrounding):
				match (trainer.Battle.BattleFormat):
					...
					PBEBattleFormat.Triple:
						...
						break;
					_: push_error("Invalid Data Exception", trainer.Battle.BattleFormat) #throw InvalidDataException(nameof(trainer.Battle.BattleFormat));
	...
	static func AreSwitchesValid(trainer : PBETrainer, switches : Array[PBESwitchIn], invalidReason : String) -> bool:
		if (trainer.Battle._battleState != PBEBattleState.WaitingForSwitchIns):
			push_error("ERR UNSPECIFIED") #throw InvalidOperationException($"{nameof(BattleState)} must be {PBEBattleState.WaitingForSwitchIns} to validate switches.");
	...
	static func IsFleeValid(trainer:PBETrainer, invalidReason:String) -> bool:
		if (trainer.Battle.BattleType != PBEBattleType.Wild):
			push_error("ERR UNSPECIFIED") #throw InvalidOperationException($"{nameof(BattleType)} must be {PBEBattleType.Wild} to flee.");
		match (trainer.Battle._battleState):
			...
			PBEBattleState.WaitingForSwitchIns:
				if (trainer.SwitchInsRequired == 0):
					invalidReason = "Switches were already submitted";
					return false;
			_: push_error("ERR UNSPECIFIED") #throw InvalidOperationException($"{nameof(BattleState)} must be {PBEBattleState.WaitingForActions} or {PBEBattleState.WaitingForSwitchIns} to flee.");
		invalidReason = null;
		return true;
...
#public sealed partial class PBEBattle
	...
	func Hit_GetVictims(user:PBEBattlePokemon, targets:Array[PBEBattlePokemon], mData:IPBEMoveData, moveType:PBEType, victims, #out List<PBEAttackVictim> 
		failFunc : Callable = null) -> void: #Func<PBEBattlePokemon, PBEResult>? 
		victims = []; victims.resize(targets.Length) #new List<PBEAttackVictim>(targets.Length);
		for target in targets:
			if (!AttackTypeCheck(user, target, moveType, result, typeEffectiveness)):
				continue
			...
			if (failFunc != null && failFunc.Invoke(target) != PBEResult.Success):
				continue
			victims.Add(PBEAttackVictim.new(target, result, typeEffectiveness));
		if (victims.Count == 0):
			return
		victims = victims.filter(func(t): not MissCheck(user, t.Pkmn, mData));
		return;
	# Outs are for hit targets that were not behind substitute
	static func Hit_HitTargets(user:PBETeam, doSub, doNormal, victims, allies, foes):
		var subAllies = victims.filter(func(v): \
				var pkmn: PBEBattlePokemon = v.Pkmn; \
				return pkmn.Team == user && pkmn.Status2.HasFlag(PBEStatus2.Substitute) )
		
		allies = victims.filter(func(v): \
			var pkmn = v.Pkmn; \
			return pkmn.Team == user && !pkmn.Status2.HasFlag(PBEStatus2.Substitute) )
		
		var subFoes = victims.filter(func(v): \
			var pkmn = v.Pkmn; \
				return pkmn.Team != user && pkmn.Status2.HasFlag(PBEStatus2.Substitute) )
		foes = victims.filter(func(v): \
			var pkmn = v.Pkmn; \
			return pkmn.Team != user && !pkmn.Status2.HasFlag(PBEStatus2.Substitute) )
		doSub(subAllies);
		doNormal(allies);
	...
	func Hit_DoCrit(victims : Array[PBEAttackVictim] ) -> void:
	...
	private void MultiHit(PBEBattlePokemon user, PBEBattlePokemon[] targets, IPBEMoveData mData, byte numHits,
		...
		while (true):
			...
			hit += 1
			if not (hit < numHits && user.HP > 0 && user.Status1 != PBEStatus1.Asleep && victims.FindIndex(v => v.Pkmn.HP > 0) != -1):
				break
	...
	private ushort DealDamage(PBEBattlePokemon culprit, PBEBattlePokemon victim, int hp, bool ignoreSubstitute = true, bool ignoreSturdy = true)
		...
		if (!ignoreSturdy && victim.HP == 0)
			...
			if (oldHP == victim.MaxHP && victim.Ability == PBEAbility.Sturdy && !culprit.HasCancellingAbility())
				sturdyHappened = true;
				victim.HP = 1;
			elif (victim.Item == PBEItem.FocusBand && _rand.RandomBool(10, 100))
				focusBandHappened = true;
				victim.HP = 1;
			elif (oldHP == victim.MaxHP && victim.Item == PBEItem.FocusSash)
...
#
	...
	#{
		...
		#{
			...
			#_:push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(battleStatusAction));
		#}
...
#
	...
	#{
		...
		#{
			...
			#_:push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(statusAction));
		#}
	...
	#{
		...
		#{
			...
			#_:push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(teamStatusAction));
		#}
		...
		#{
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(ap.AbilityAction));
						#}
						#break;
					#}
					...
					#{
						...
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(ap.Ability));
				#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(bsp.BattleStatusAction));
						#}
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(bsp.BattleStatus));
				#}
				#return message;
			#}
			#PBECapturePacket cp:
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(ip.ItemAction));
						#}
						#break;
					#}
					#PBEItem.ApicotBerry:
					...
					#{
						...
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(ip.Item));
				#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(itp.Item));
						#}
						##return string.Format(message, GetPkmnName(itemUser, true), itemEnglish);
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(itp.ItemAction));
				#}
			#}
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(mrp.Result));
				#}
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(pscp.Stat));
				#}
				...
				#{
					...
					#_:
						...
						#elif (change == 0 && pscp.NewValue == battle.Settings.MaxStatChange)
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(smp.Message));
				#}
				#return message;
			#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(s1p.StatusAction));
						#}
						#break;
					#}
					...
					#{
						...
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(s1p.Status1));
				#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(s2p.StatusAction));
						#}
						#break;
					#}
					#PBEStatus2.Confused:
					...
					#{
						...
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(s2p.Status2));
				#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(tsp.TeamStatusAction));
						#}
						#break;
					#}
					...
					#{
						...
						#break;
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(tsp.TeamStatus));
				#}
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(tsdp.TeamStatus));
				#}
				...
				#{
					...
					#{
						...
						#{
							...
							#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(wp.WeatherAction));
						#}
					#}
					...
					#{
						...
						#}
					#}
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(wp.Weather));
				#}
			#}
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(wdp.Weather));
				#}
			...
			#{
				...
				#{
					...
					#_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(brp.BattleResult));
				#}
...
#public sealed partial class PBEBattle
	...
	private void CheckCanSaveReplay()
		if (!IsLocallyHosted)
			push_error("ERR UNSPECIFIED") #throw InvalidOperationException("Can only save replays of locally hosted battles");
		if (_battleState != PBEBattleState.Ended)
			push_error("ERR UNSPECIFIED") #throw InvalidOperationException($"{nameof(BattleState)} must be {PBEBattleState.Ended} to save a replay.");
		...
		using (var s = new MemoryStream(fileBytes))
		{
			...
			for (int i = 0; i < 16; i++)
				if (hash[i] != fileBytes[fileBytes.Length - 16 + i])
					push_error("ERR UNSPECIFIED") #throw InvalidDataException();
...
#public sealed partial class PBEBattle
		...
		match (battleFormat):
			...
			PBEBattleFormat.Rotation:
				if (position == PBEFieldPosition.Center):
					...
					push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(position));
			PBEBattleFormat.Double:
				if (position == PBEFieldPosition.Left):
					return PBEFieldPosition.Right;
				elif (position == PBEFieldPosition.Right):
			...
			PBEBattleFormat.Triple:
				if (position == PBEFieldPosition.Left)
					return PBEFieldPosition.Right;
				elif (position == PBEFieldPosition.Center)
					return PBEFieldPosition.Center;
				elif (position == PBEFieldPosition.Right)
					return PBEFieldPosition.Left;
				else
					push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(position));
			_:push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(battleFormat));
	...
	public static IReadOnlyList<PBEBattlePokemon> GetRuntimeSurrounding(PBEBattlePokemon pkmn, bool includeAllies, bool includeFoes)
	{
		...
		{
			push_error("ERR UNSPECIFIED") #throw ArgumentException($"\"{nameof(includeAllies)}\" and \"{nameof(includeFoes)}\" were false.");
		}
		...
		match (pkmn.Battle.BattleFormat)
		{
			PBEBattleFormat.Single:
			{
				...
				{
					push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(pkmn.FieldPosition));
			PBEBattleFormat.Double:
				if (pkmn.FieldPosition == PBEFieldPosition.Left)
					...
					if (includeFoes)
						...
						return ret;
					return Array.Empty<PBEBattlePokemon>();
				elif (pkmn.FieldPosition == PBEFieldPosition.Right)
					List<PBEBattlePokemon> ret = null!;
					if (includeAllies)
			...
			_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(pkmn.Battle.BattleFormat));
		}
	}
	...
	private static void FindFoeLeftTarget(PBEBattlePokemon user, bool canHitFarCorners, List<PBEBattlePokemon> targets)
	{
		...
		if (!ot.TryGetPokemon(PBEFieldPosition.Left, out PBEBattlePokemon? pkmn))
		{
			...
			match (user.Battle.BattleFormat)
			{
				...
				_:push_error("ERR UNSPECIFIED") #throw InvalidOperationException();
			}
		}
		targets.Add(pkmn);
	}
			...
			match (user.Battle.BattleFormat)
			{
				...
				PBEBattleFormat.Triple:
				{
					...
					match (user.FieldPosition)
					{
						...
						_:push_error("ERR UNSPECIFIED") #throw InvalidDataException();
					}
					break;
				}
			}
	...
	public static bool AreTargetsValid(PBEBattlePokemon pkmn, PBEMove move, PBETurnTarget targets)
	{
		...
		{
			push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(move));
		}
		...
		match (pkmn.Battle.BattleFormat)
		{
			PBEBattleFormat.Single:
			{
				match (possibleTargets)
				{
					...
					_:push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(possibleTargets));
				}
			}
			PBEBattleFormat.Double:
			{
				match (possibleTargets)
				{
					...
					_:push_error("ERR UNSPECIFIED") #push_error("ERR UNSPECIFIED") #throw InvalidDataException(nameof(possibleTargets));
				}
			}
			PBEBattleFormat.Triple:
			{
	...
	public static PBETurnTarget GetRandomTargetForMetronome(PBEBattlePokemon pkmn, PBEMove calledMove, PBERandom rand)
	{
		...
		{
			push_error("ERR UNSPECIFIED") #throw ArgumentOutOfRangeException(nameof(calledMove));
		}
