...
class_name PBEBattlePokemon# : IPBEPokemonKnownTypes, IPBEPokemonTypes, IPBESpeciesForm
...
var OriginalEffortValues : PBEReadOnlyStatCollection
var EffortValues : PBEStatCollection
var IndividualValues : PBEReadOnlyStatCollection
var Friendship : int
...
var InfatuatedWithPokemon : PBEBattlePokemon
...
var LockOnPokemon : PBEBattlePokemon
var LockOnTurns : int 
...
var TurnAction : PBETurnAction
## <summary>The move the Pokémon is forced to use by multi-turn moves.</summary>
var TempLockedMove : PBEMove  # TODO: Tests - Does a pkmn lose its temp locked move if it runs out of pp on the move, all moves, or ever? (Some move can lower its pp while it's being used?)
## <summary>The targets the Pokémon is forced to target by multi-turn moves.</summary>
var TempLockedTargets : PBETurnTarget 
## <summary>The move the Pokémon is forced to use by its choice item.</summary>
var ChoiceLockedMove : PBEMove # TODO: Tests - Does a pkmn lose its choice locked move if it runs out of pp on the move, all moves, or ever?
...
var SpeedBoost_AbleToSpeedBoostThisTurn : bool

#endregion

var EXPPokemon # public List<PBEBattlePokemon> = new();

#region Constructors

func _init(trainer:PBETrainer, id:int, \
	species:PBESpecies, form:PBEForm, nickname:String, level:int, exp:int, friendship:int, shiny:bool, pokerus:bool, \
	ability:PBEAbility, nature:PBENature, gender:PBEGender, item:PBEItem, caughtBall:PBEItem, \
	evs:PBEReadOnlyStatCollection, ivs:PBEReadOnlyStatCollection, moves:PBEReadOnlyPartyMoveset):
	...
	KnownForm = form;
	RevertForm = form;
	var pData : IPBEPokemonData = PBEDataProvider.Instance.GetPokemonData(species, form);
	...
	Item = item;
	OriginalItem = item;
	KnownItem = 9223372036854775807 # int max
	KnownCaughtBall = caughtBall;
	CaughtBall = caughtBall;
	OriginalCaughtBall = caughtBall;
	OriginalEffortValues = evs;
	EffortValues = PBEStatCollection.new(evs);
	IndividualValues = PBEReadOnlyStatCollection.new(ivs);
	SetStats(true, true);
	OriginalMoveset = moves;
	var settings : PBESettings = Battle.Settings;
	Moves = PBEBattleMoveset.new(settings, moves);
	KnownMoves = PBEBattleMoveset.new(settings);
	TransformBackupMoves = PBEBattleMoveset.new(settings);
...
public void ClearForSwitch()
{
	...
	elif (Status1 == PBEStatus1.BadlyPoisoned)
	{
		Status1Counter = 1;
	}
	ResetVolatileStuff();
...
public void SetStats(bool calculateHP, bool setMaxHPIfCalcHP)
{
	...
	if (calculateHP)
	{
		...
		elif (HP != 0 && oldMaxHP != hp) # Don't change for a fainted mon
		{
...
public void UpdateKnownPP(PBEMove move, int amountReduced)
{
	...
	if (knownSlot.MaxPP == 0)
	{
		...
		elif (Battle.Settings.MaxPPUps == 0 || knownSlot.PP > PBEBattleMoveset.GetNonTransformPP(Battle.Settings, move, (byte)(Battle.Settings.MaxPPUps - 1)))
		{
...
public sbyte GetStatChange(PBEStat stat)
{
	match (stat)
	{
		PBEStat.Attack: return AttackChange;
		PBEStat.Defense: return DefenseChange;
		PBEStat.SpAttack: return SpAttackChange;
		PBEStat.SpDefense: return SpDefenseChange;
		PBEStat.Speed: return SpeedChange;
		PBEStat.Accuracy: return AccuracyChange;
		PBEStat.Evasion: return EvasionChange;
...
public PBEResult IsStatChangePossible(PBEStat stat, PBEBattlePokemon causer, int change, out sbyte oldValue, out sbyte newValue, bool useKnownInfo = false, bool ignoreSubstitute = false)
{
	...
	{
		match (useKnownInfo ? KnownAbility : Ability)
		{
			PBEAbility.BigPecks:
			{
	...
	if (causer == this || !causer.HasCancellingAbility())
	{
		match (useKnownInfo ? KnownAbility : Ability)
		{
			PBEAbility.Contrary: change *= -1
			PBEAbility.Simple: change *= 2
		}
	}
...
public PBEType GetMoveType(IPBEMoveData mData, bool useKnownInfo = false)
{
	...
	match (mData.Effect)
	{
		...
		{
			match (useKnownInfo ? KnownItem : Item)
			{
				PBEItem.DracoPlate: return PBEType.Dragon;
				PBEItem.DreadPlate: return PBEType.Dark;
				PBEItem.EarthPlate: return PBEType.Ground;
				PBEItem.FistPlate: return PBEType.Fighting;
				PBEItem.FlamePlate: return PBEType.Fire;
				PBEItem.IciclePlate: return PBEType.Ice;
				PBEItem.InsectPlate: return PBEType.Bug;
				PBEItem.IronPlate: return PBEType.Steel;
				PBEItem.MeadowPlate: return PBEType.Grass;
				PBEItem.MindPlate: return PBEType.Psychic;
				PBEItem.SkyPlate: return PBEType.Flying;
				PBEItem.SplashPlate: return PBEType.Water;
				PBEItem.SpookyPlate: return PBEType.Ghost;
				PBEItem.StonePlate: return PBEType.Rock;
				PBEItem.ToxicPlate: return PBEType.Poison;
				PBEItem.ZapPlate: return PBEType.Electric;
			}
			break;
		}
		PBEMoveEffect.Struggle:
		{
			return PBEType.None;
		}
		PBEMoveEffect.TechnoBlast:
		{
			match (useKnownInfo ? KnownItem : Item)
			{
				PBEItem.BurnDrive: return PBEType.Fire;
				PBEItem.ChillDrive: return PBEType.Ice;
				PBEItem.DouseDrive: return PBEType.Water;
				PBEItem.ShockDrive: return PBEType.Electric;
			}
			break;
		}
		PBEMoveEffect.WeatherBall:
		{
			if (Battle.ShouldDoWeatherEffects())
			{
				...
				{
					PBEWeather.Hailstorm: return PBEType.Ice;
					PBEWeather.HarshSunlight: return PBEType.Fire;
					PBEWeather.Rain: return PBEType.Water;
					PBEWeather.Sandstorm: return PBEType.Rock;
				}
			}
			break;
		}
...
public bool IsForcedToStruggle()
{
	...
	elif ((ChoiceLockedMove != PBEMove.None && Moves[ChoiceLockedMove]!.PP == 0) # If the choice locked move has 0 pp, it is forced to struggle
...
public PBEMove[] GetUsableMoves():
	...
	elif (TempLockedMove != PBEMove.None)
		return new PBEMove[1] { TempLockedMove };
	elif (ChoiceLockedMove != PBEMove.None)
