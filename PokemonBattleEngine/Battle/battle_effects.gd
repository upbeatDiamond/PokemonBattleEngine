...
var _calledFromOtherMove := false;


func DoSwitchInEffects(battlers : Array[PBEBattlePokemon], forcedInBy : PBEBattlePokemon = null):
	# Set EXP Seens first
	for pkmn in battlers:
		var opTeam : PBETeam = pkmn.Team.OpposingTeam;
		for op in opTeam.ActiveBattlers:
			op.AddEXPPokemon(pkmn);
			pkmn.AddEXPPokemon(op);
	
	var order : Array[PBEBattlePokemon] = GetActingOrder(battlers, true)
	
	for pkmn in order:
		var grounded : bool = pkmn.IsGrounded(forcedInBy) == PBEResult.Success;
		...
		if (grounded && pkmn.Team.TeamStatus.HasFlag(PBETeamStatus.Spikes)):
			...
			if (FaintCheck(pkmn)):
				continue;
			LowHPBerryCheck(pkmn, forcedToEatBy);
		...
		if (grounded && pkmn.Team.TeamStatus.HasFlag(PBETeamStatus.ToxicSpikes)):
			...
			elif (pkmn.IsPoisonPossible(forcedInBy, ignoreSubstitute: true) == PBEEnums.PBEResult.Success):
...
func DoPostHitEffects(PBEBattlePokemon user, PBEBattlePokemon victim, IPBEMoveData mData, PBEType moveType)
{
...
func DoPostAttackedEffects(PBEBattlePokemon user, List<PBEAttackVictim> allies, List<PBEAttackVictim> foes, bool doLifeOrb,
...
func WildFleeCheck(PBEBattlePokemon pkmn)
{
	# TODO: Trapping effects
	SetEscaped(pkmn);
}

func CalcEXP(PBEBattlePokemon loser)
{
...
func GiveEXP(PBEBattlePokemon victor, uint amount)
{
...
func GenerateCapture(PBEBattlePokemon user, PBEBattlePokemon wildPkmn, PBEItem ball, out byte shakes, out bool success, out bool isCriticalCapture)
{
...
func UseItem(PBEBattlePokemon user, PBEItem item)
{
...
func UseMove(PBEBattlePokemon user, PBEMove move, PBETurnTarget requestedTargets)
{
...
func TrySetLoser(PBEBattlePokemon pkmn)
{
...
func SetEscaped(PBEBattlePokemon pkmn)
{
...
func ActivateAbility(PBEBattlePokemon pkmn, bool switchIn)
{
	if (!switchIn)
	{
...
func CastformCherrimCheckAll()
{
	CastformCherrimCheck(GetActingOrder(ActiveBattlers, true));
}
func CastformCherrimCheck(IEnumerable<PBEBattlePokemon> order)
{
	foreach (PBEBattlePokemon pkmn in order)
	{
...
func CastformCherrimCheck(PBEBattlePokemon pkmn)
{
	if (pkmn.HP == 0)
	{
...
func ShayminCheck(PBEBattlePokemon pkmn)
{
...
func IllusionBreak(PBEBattlePokemon pkmn, PBEBattlePokemon breaker)
{
...
func AntiStatusAbilityCheck(IEnumerable<PBEBattlePokemon> order)
{
	foreach (PBEBattlePokemon pkmn in order)
	{
		AntiStatusAbilityCheck(pkmn);
...
func AntiStatusAbilityCheck(PBEBattlePokemon pkmn)
{
	switch (pkmn.Ability)
	{
...
func CauseConfusion(PBEBattlePokemon target, PBEBattlePokemon other)
{
...
func CauseInfatuation(PBEBattlePokemon target, PBEBattlePokemon other)
{
...
func WhiteHerbCheck(PBEBattlePokemon pkmn)
{
...
func LowHPBerryCheckArray(order : Array[PBEBattlePokemon], forcedToEatBy:PBEBattlePokemon = null):
	for pkmn in order:
		LowHPBerryCheck(pkmn, forcedToEatBy);


func LowHPBerryCheck(pkmn : PBEBattlePokemon, forcedToEatBy:PBEBattlePokemon = null)
	forcedToEatBy = forcedToEatBy if forcedToEatBy != null else pkmn;
	
	func DoConfuseBerry(flavor : PBEFlavor):
	...
	func DoHealItem(int hp):
		BroadcastItem(pkmn, forcedToEatBy, pkmn.Item, PBEItemAction.Consumed)
		HealDamage(pkmn, hp)
	
	func DoStatItem(PBEStat stat, int change):
	...
	if (pkmn.HP <= pkmn.MaxHP / 4):
		match (pkmn.Item):
			PBEItem.ApicotBerry: 
				DoStatItem(PBEStat.SpDefense, +1)
			PBEItem.GanlonBerry: 
				DoStatItem(PBEStat.Defense, +1)
			PBEItem.LiechiBerry: 
				DoStatItem(PBEStat.Attack, +1)
			PBEItem.PetayaBerry: 
				DoStatItem(PBEStat.SpAttack, +1)
			PBEItem.SalacBerry: 
				DoStatItem(PBEStat.Speed, +1)
			PBEItem.StarfBerry:
				# Verified: Starf Berry does not activate for Accuracy or Evasion, or if all other stats are maximized
				var statsThatCanGoUp : Array[PBEStat] = PBEDataUtils.StarfBerryStats.FindAll(s => pkmn.GetStatChange(s) < Settings.MaxStatChange);
...
func SetAbility(PBEBattlePokemon user, PBEBattlePokemon target, PBEAbility ability):
...
func SetWeather(PBEWeather weather, byte weatherCounter, bool switchIn):
	Weather = weather;
...
func RecordExecutedMove(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData):
...
func PPReduce(PBEBattlePokemon pkmn, PBEMove move):
...
func static ApplyBigRoot(PBEBattlePokemon pkmn, ref int restoreAmt):
	if (pkmn.Item == PBEItem.BigRoot):
		restoreAmt += (int)(restoreAmt * 0.3)


func CureNightmare(PBEBattlePokemon wakingUp, PBEBattlePokemon pokemon2):
...
func SetSleepTurns(PBEBattlePokemon pkmn, int minTurns, int maxTurns):
	pkmn.SleepTurns = (byte)(_rand.RandomInt(minTurns, maxTurns) / (pkmn.Ability == PBEAbility.EarlyBird ? 2 : 1));


func DoTransform(PBEBattlePokemon user, PBEBattlePokemon target):
...
func ApplyStatus1IfPossible(PBEBattlePokemon user, PBEBattlePokemon target, PBEStatus1 status, bool broadcastUnsuccessful) -> PBEResult:
	PBEResult result;
	switch (status)
	{
...
func ApplyStatChangeIfPossible(PBEBattlePokemon user, PBEBattlePokemon target, PBEStat stat, int change, bool isSecondaryEffect = false)
{
...
func SetStatAndBroadcast(PBEBattlePokemon pkmn, PBEStat stat, sbyte oldValue, sbyte newValue)
{
...
func SwitchTwoPokemon(PBEBattlePokemon pkmnLeaving, PBEBattlePokemon pkmnComing, PBEBattlePokemon? forcedByPkmn = null)
{
...
func RemoveInfatuationsAndLockOns(PBEBattlePokemon pkmnLeaving)
{
...
func SemiInvulnerableChargeMove(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBETurnTarget requestedTargets, PBEStatus2 status2,
...
func Ef_TryForceStatus1(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBEStatus1 status, bool thunderWave = false)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_TryForceStatus2(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBEStatus2 status)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_TryForceBattleStatus(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData, PBEBattleStatus status)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	switch (status)
...
func Ef_TryForceTeamStatus(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData, PBETeamStatus status)
{
	PBEResult result;
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_TryForceWeather(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData, PBEWeather weather)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (Weather == weather)
...
func Ef_Growth(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
...
func Ef_ChangeTargetStats(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, (PBEStat, int)[] statChanges, bool requireAttraction = false)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Hit__MaybeChangeTargetStats(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, (PBEStat, int)[] statChanges, int chanceToChangeStats)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Hit__MaybeChangeUserStats(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, (PBEStat, int)[] statChanges, int chanceToChangeStats)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Entrainment(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
...
func Ef_RolePlay(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
...
func Ef_SetOtherAbility(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBEAbility ability, bool blockedByTruant)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Bounce(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBETurnTarget requestedTargets)
{
	void BeforePostHit(PBEBattlePokemon target, ushort damageDealt)
	{
...
func Ef_ShadowForce(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBETurnTarget requestedTargets)
{
...
func Ef_BrickBreak(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Feint(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Hit(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBEStatus1 status1 = PBEStatus1.None, int chanceToInflictStatus1 = 0, PBEStatus2 status2 = PBEStatus2.None, int chanceToInflictStatus2 = 0)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Hit__MaybeBurnFreezeParalyze(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_MultiHit(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, byte numHits, bool subsequentMissChecks = false, PBEStatus1 status1 = PBEStatus1.None, int chanceToInflictStatus1 = 0)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_MultiHit_2To5(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	byte numHits;
...
func Ef_PayDay(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Recoil(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, PBEStatus1 status1 = PBEStatus1.None, int chanceToInflictStatus1 = 0, PBEStatus2 status2 = PBEStatus2.None, int chanceToInflictStatus2 = 0)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_SecretPower(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Selfdestruct(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	# TODO: Damp
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_SmellingSalt(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Snore(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
...
func Ef_Struggle(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
...
func Ef_SuckerPunch(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_WakeUpSlap(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Endeavor(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_FinalGambit(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_OneHitKnockout(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Psywave(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_SeismicToss(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_SetDamage(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_SuperFang(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_HPDrain(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData, bool requireSleep = false)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Moonlight(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_PainSplit(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Rest(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (user.Status1 == PBEStatus1.Asleep)
...
func Ef_RestoreTargetHP(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Roost(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_BellyDrum(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Camouflage(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Conversion(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Curse(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Flatter(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Haze(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_HelpingHand(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Metronome(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_Nothing(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_PsychUp(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_ReflectType(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Refresh(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Soak(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Swagger(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
...
func Ef_Teleport(PBEBattlePokemon user, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
...
func Ef_Whirlwind(PBEBattlePokemon user, PBEBattlePokemon[] targets, PBEMove move, IPBEMoveData mData)
{
	BroadcastMoveUsed(user, move);
	PPReduce(user, move);
	if (targets.Length == 0)
