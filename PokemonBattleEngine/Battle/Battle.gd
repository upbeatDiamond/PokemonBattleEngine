...
func _init_from_packet(packet:PBEBattlePacket) -> PBEBattle:
	var battle = PBEBattle.new( BattleType. )
	battle.IsLocallyHosted = false;
	battle.BattleType = packet.BattleType;
	battle.BattleFormat = packet.BattleFormat;
	battle.BattleTerrain = packet.BattleTerrain;
	battle.Weather = packet.Weather;
	battle.Settings = packet.Settings;
...
func IEnumerable<PBEBattlePokemon> GetActingOrder(IEnumerable<PBEBattlePokemon> pokemon, bool ignoreItemsThatActivate)
{
